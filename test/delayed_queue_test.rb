...
context "DelayedQueue" do
  ...
  test "enqueue_at adds correct list and zset" do
    ...
    assert_equal(0, ResqueScheduler.redis.llen("delayed:#{timestamp.to_i}").to_i, "delayed queue should be empty to start")

    Resque.enqueue_at(timestamp, SomeIvarJob, "path")

    # Confirm the correct keys were added
    assert_equal(1, ResqueScheduler.redis.llen("delayed:#{timestamp.to_i}").to_i, "delayed queue should have one entry now")
    assert_equal(1, ResqueScheduler.redis.zcard(:delayed_queue_schedule), "The delayed_queue_schedule should have 1 entry now")
    ...
    assert(!ResqueScheduler.redis.exists("delayed:#{timestamp.to_i}"))
    assert_equal(0, ResqueScheduler.redis.zcard(:delayed_queue_schedule), "delayed queue should be empty")
  end
  ...
  test "enqueue_at and enqueue_in are equivelent" do
    ...
    assert_equal(1, ResqueScheduler.redis.zcard(:delayed_queue_schedule), "should have one timestamp in the delayed queue")
    assert_equal(2, ResqueScheduler.redis.llen("delayed:#{timestamp.to_i}"), "should have 2 items in the timestamp queue")
  end
  ...
  test "remove_delayed_job_from_timestamp should cleanup the delayed timestamp list if not jobs are left" do
    ...
    Resque.enqueue_at(t, SomeIvarJob, "foo")
    assert_equal 1, Resque.remove_delayed_job_from_timestamp(t, SomeIvarJob, "foo")
    assert !ResqueScheduler.redis.exists("delayed:#{t.to_i}")
